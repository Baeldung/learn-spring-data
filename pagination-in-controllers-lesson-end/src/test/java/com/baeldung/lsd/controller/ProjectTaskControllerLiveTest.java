...
class ProjectTaskControllerLiveTest {
    ...
    @Test
    void givenPaginationInfo_whenGetProjectTasks_thenSuccess() throws Exception {

        mockMvc.perform(get("/projects/1/tasks").param("page", "5")
            .param("size", "10")
            .param("sort", "name,desc"))
            .andExpect(status().isOk());

        ArgumentCaptor<Pageable> pageableCaptor = ArgumentCaptor.forClass(Pageable.class);
        verify(taskRepository).findByProjectId(Mockito.anyLong(), pageableCaptor.capture());
