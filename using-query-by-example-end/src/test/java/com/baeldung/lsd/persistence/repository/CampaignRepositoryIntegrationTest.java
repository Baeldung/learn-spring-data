...
class CampaignRepositoryIntegrationTest {
    ...
    @Test
    void givenCampaignExample_whenQueryByExample_thenSuccess() {
        Campaign probe = new Campaign();
        probe.setName("Campaign 1");

        Example<Campaign> campaignExample = Example.of(probe);
        Optional<Campaign> dbCampaign = campaignRepository.findOne(campaignExample);

        assertTrue(dbCampaign.isPresent());
        assertThat(dbCampaign.get()
            .getName()).isEqualTo("Campaign 1");
    }

    @Test
    void givenCampaignExample_whenMatchingStartsWithIgnoreCase_thenSuccess() {
        Campaign probe = new Campaign();
        probe.setName("campaign");

        Example<Campaign> campaignExample = Example.of(probe, ExampleMatcher.matching()
            .withMatcher("name", matcher -> matcher.startsWith()
                .ignoreCase()));

        assertThat(campaignRepository.findAll(campaignExample)).hasSize(3);
    }
